...
// Added by: Staffan
// Definition of a Microchip CAN mask
typedef union __MASK
{
	unsigned long dword;
	struct __BYTES
	{
...
_MASK           _uCANRxMasks[CAN_MAX_RCV_ENDP];
...
UNSIGNED32 _uCAN_accepted_COBID;
signed long ACCEPTED_COBID;
...
void _CANReset(void)
{
	...
	MSEL0 = MSEL1 = MSEL2 = MSEL3 = 0; //orig
        //MSEL0 = MSEL1 = MSEL2 = MSEL3 = 0xFF; //Do not mask anything.
	
	// Set Mask 0 to mask all.
        RXM0SIDH = 0x00;
        RXM0SIDL = 0x00;
        RXM0EIDH = 0x00;
        RXM0EIDL = 0x00;
...
void _CANIsGetReady(void)
{
	...
	{				
		...
		_uCAN_ret = *((RXB0CON & 0x0F) + _uCANRxHndls);
                ...
                mTOOLS_MCHP2CO(*(unsigned long*) mCANGetPtrRxCOB());
                _uCAN_accepted_COBID.word = mTOOLS_GetCOBID();
                ACCEPTED_COBID = _uCAN_accepted_COBID.word;
                MESSAGE_RECEIVED = 1;
	}
	else _uCAN_ret = 0;
}
...
void _CANIsGetReady2(void)
{
	...
	if (PIR3bits.RXB1IF)
	{
		...
		// Decode which filter passed the event and return the handle
		// associated to the filter
                // Staffan detta måste ändras ty vi kommer ej använda filter.
                // men måste avkoda meddelandets typ ändå.
		_uCAN_ret = *((RXB0CON & 0x0F) + _uCANRxHndls);
	}
	else _uCAN_ret = 0;
}
